...
- **NAS 백업 파이프라인 구축**(Tailscale + scp, 30일 보관) 및 디스크 회수 자동화(100%→72%, 15GB)
...
- 데이터 적재 잡 스케줄링·스로틀링(법안 1,213건 적재, 이미지 in-place 압축 323MB→13MB)
...
> 확인 필요(지원 전): 병역 이행 여부(남성 필수요건), 근무지(판교 삼평동) 통근.
